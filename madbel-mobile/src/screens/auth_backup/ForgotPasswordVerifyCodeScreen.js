...
const ForgotPasswordVerifyCodeScreen = () => {
  ...
  const [forgotPasswordVerifyCode, { isLoading: verifyLoading }] =
  ...
  const handleVerify = async (data) => {
    try {
      const response = await forgotPasswordVerifyCode({
        email: forgotEmail,
        otp: data?.otp,
      }).unwrap();
        ...
        <LinearGradient colors={["#02080B", "#010406"]} style={styles.screen}>
          ...
          <ScrollView contentContainerStyle={styles.content}>
            <View style={styles.heroWrap}>
              ...
              <Text style={styles.subtitle}>{t("enter_the_6_digit_code_sent_to_your_email")}</Text>
            </View>
            ...
            {errors?.root?.type === "forgotPasswordVerifyCode" && (
            ...
            >
              <Text style={styles.bottomHint}>{t("i_didn_t_receive_a_code")}</Text>
            </Pressable> */}
          </ScrollView>
        </LinearGradient>
      </SafeAreaView>
