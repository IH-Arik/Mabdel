...
//     return formatMeetingDateTime(event?.starts_at, event?.ends_at);
...
//         Alert.alert(
//           t("share_unavailable"),
//           t("could_not_generate_a_share_link"),
//         );
//         return;
//       }
...
//       Alert.alert(
//         "Share failed",
//         error?.data?.message || "Could not share this meeting.",
//       );
//     } finally {
...
//             <ChevronLeft size={responsiveWidth(5)} color="#F2F8FF" />
//           </Pressable>
...
//               icon={MapPin}
//               title={
//                 event?.meeting_mode === "online" ? "Meeting Link" : "Location"
//               }
//               value={
//                 event?.meeting_mode === "online"
...
//   },
//   headerTitle: { color: "#F2F8FF", fontSize: responsiveWidth(5), fontWeight: "700" },
//   meetingTitle: {
//     color: "#F4F9FF",
...
//     borderColor: "#0E5D70",
//   },
//   actionText: { color: "#F2F8FF", fontSize: responsiveWidth(4), fontWeight: "700" },
...
const MeetingDetailsScreen = () => {
  ...
  const event = eventResponse?.data || route?.params?.event || null;
  const googleCalendarEvent = route?.params?.googleCalendarEvent || null;
  const appleCalendarEvent = route?.params?.appleCalendarEvent || null;
      ...
      <View style={styles.container}>
        ...
        {isLoading ? (
          ...
          >
            ...
            {appleCalendarEvent?.id ? (
              <View style={styles.appleCard}>
                <Text style={styles.appleLabel}>Apple Calendar</Text>
                <Text style={styles.appleValue}>Synced successfully</Text>
                <Text style={styles.appleLink} numberOfLines={2}>
                  {appleCalendarEvent.calendarName
                    ? `Saved to ${appleCalendarEvent.calendarName}`
                    : `Calendar event ID: ${appleCalendarEvent.id}`}
                </Text>
              </View>
            ) : null}
...
const styles = StyleSheet.create({
  ...
  appleLabel: { color: "#FFD39A", fontSize: 12, fontWeight: "700" },
  appleValue: { color: "#F2F8FF", fontSize: 15, fontWeight: "700" },
  appleLink: { color: "#B8A995", fontSize: 12, lineHeight: 18 },
  actionsRow: {
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "space-between",
