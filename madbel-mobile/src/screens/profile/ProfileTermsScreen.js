...
import { useMadbelGetTermsAndConditionsQuery } from "../../redux/slices/madbelApiSlice";

const ProfileTermsScreen = () => {
  const { t } = useAppLanguage();
  const navigation = useNavigation();
  const { data: termsResponse, isLoading } = useMadbelGetTermsAndConditionsQuery(undefined, {
    refetchOnMountOrArgChange: true,
  });

  const blocks = termsResponse?.data?.blocks || [];
    ...
    <SafeAreaView style={styles.safeArea}>
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>{t("terms_condition")}</Text>
