...
// //       .map((item) => toUiMessage(item, myUserId))
...
// //         orderedMessages[index + 1]?.sender !== "me");

// //     const showDateSeparator =
// //       index === 0 ||
// //       new Date(item?.raw?.createdAt || 0).toDateString() !==
// //         new Date(orderedMessages[index - 1]?.raw?.createdAt || 0).toDateString();
...
// //                 </Text>
// //                 <Text
// //                   style={{ color: "#8A9AB0", fontSize: 13 }}
// //                   numberOfLines={1}
// //                 >
// //                   {replyToMessage.text}
// //                 </Text>
...
// //               >
// //                 <Text
// //                   style={{ color: "#5D6A7A", fontSize: 20, lineHeight: 20 }}
// //                 >
// //                   ✕
// //                 </Text>
// //               </Pressable>
...
// const DEFAULT_HEADER_HEIGHT = Platform.OS === "ios" ? 90 : 0;
...
//     // FIX: guard against missing createdAt so a message without a timestamp
//     // doesn't fall back to 1970 and trigger a spurious date separator.
//     const currentHasDate = Boolean(item?.raw?.createdAt);
//     const prevItem = orderedMessages[index - 1];
//     const prevHasDate = Boolean(prevItem?.raw?.createdAt);
//     const showDateSeparator =
//       currentHasDate &&
//       (index === 0 ||
//         !prevHasDate ||
//         new Date(item.raw.createdAt).toDateString() !==
//           new Date(prevItem.raw.createdAt).toDateString());
...
//         behavior={Platform.OS === "ios" ? "padding" : "height"}
//         style={{ flex: 1 }}
//         // FIX: use the actually-measured header height instead of a magic
//         // number, so this doesn't silently go stale if the header layout
//         // changes (e.g. group subtitle adding a line).
...
//               // out its new content, so it can't fire too early.
//               onContentSizeChange={() => {
...
//             </Text>
//             <FlatList
//               data={allThreads.filter((th) => (th._id || th.id) !== threadId)}
//               keyExtractor={(item) => String(item._id || item.id)}
//               renderItem={({ item }) => (
//                 <Pressable
//                   style={{
...
const SingleChatScreen = () => {
  ...
  // sendMessage now optionally accepts overrideText so the voice-note
  // flow can reuse the exact same send path as typed messages.
  const sendMessage = async (overrideText) => {
    ...
    try {
      let created;
      if (pendingReply && overrideText === undefined) {
        const result = await replyToMessageMutation({
          message_id: pendingReply.id,
          content: text,
          platform: "ai",
  ...
  const startRecordingTimer = () => {
    ...
    recordingTimerRef.current = setInterval(() => {
      setRecordingElapsedMs(Date.now() - (recordingStartRef.current || Date.now()));
    }, 250);
  };
  ...
  const handleRecordButtonPress = async () => {
    if (recorderState?.isRecording) {
      try {
        await recorder.stop();
        const finalDuration = stopRecordingTimer();
        const uri = recorder.uri;
        if (uri) {
          setRecordedUri(uri);
          setRecordedDurationMs(finalDuration);
        } else {
          Alert.alert(t("error"), t("could_not_save_recording"));
        }
      } catch (error) {
        stopRecordingTimer();
        Alert.alert(t("error"), t("could_not_stop_recording"));
      }
    } else {
      try {
        const permission = await requestRecordingPermissionsAsync();
        if (!permission.granted) {
          Alert.alert(
            ...
            t("enable_mic_permission_in_settings"),
          );
          return;
        }
        await setAudioModeAsync({
  ...
  const handleSendVoiceMessage = async () => {
    if (!recordedUri || !threadId || isSendingVoiceMessage) return;
    try {
      previewPlayer.pause();

      const fetchResponse = await fetch(recordedUri);
      const blob = await fetchResponse.blob();
      ...
      const result = await transcribeVoice({
        audio_base64: audioBase64,
        audio_filename: `voice-${Date.now()}.m4a`,
        audio_mime_type: "audio/m4a",
      }).unwrap();

      const transcript = (result?.data || result)?.transcript;
      if (!transcript) {
        Alert.alert(t("error"), t("could_not_transcribe_voice"));
        return;
      }
      ...
      Alert.alert(t("error"), t("could_not_send_voice_message"));
    }
  };
  ...
  const previewProgressPercent = previewPlayerStatus?.duration
    ? Math.min(
        100,
        ((previewPlayerStatus?.currentTime || 0) / previewPlayerStatus.duration) * 100,
      )
    : 0;

  const previewTimeLabelMs = previewPlayerStatus?.playing
    ? (previewPlayerStatus?.currentTime || 0) * 1000
    : recordedDurationMs;
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#020406" }}>
      ...
      >
        ...
        >
          ...
          {shouldShowSuggestions && !recordedUri && !recorderState?.isRecording && (
            <View
              style={{
                flexDirection: "row",
                gap: responsiveWidth(2),
                ...
                >
                  ...
                  >
                    {previewPlayerStatus?.playing ? (
                      <Pause size={20} color="#17CBE8" />
                    ) : (
                      <Play size={20} color="#17CBE8" />
                    )}
                  </Pressable>
                  ...
                  <Text
                    style={{ color: "#8A9AB0", fontSize: 12, fontWeight: "600" }}
                  >
                    {formatDuration(previewTimeLabelMs)}
                  </Text>
                </View>
                ...
                >
                  {recorderState?.isRecording ? (
                    ...
                    >
                      ...
                      >
                        {t("recording")} {formatDuration(recordingElapsedMs)}
                      </Text>
                    </View>
                  ) : (
                    <TextInput
