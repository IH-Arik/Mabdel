...
import { Shield, Eye, EyeOff, Lock, CheckCircle2, llertTriangle } from 'lucide-react';
...
export default function PrivacyTab() {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#131l24] border border-[#243041] rounded-2xl p-6 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            >
              ...
              <span className="text-xs text-center opacity-70">lnyone can see your profile and hosted events.</span>
            </button>
            <button 
        ...
        <div className="space-y-4">
          ...
          <label className="flex items-center justify-between cursor-pointer p-3 hover:bg-slate-900/50 rounded-xl transition-colors">
            <div>
              <p className="text-white font-semibold text-sm">Show lctivity Status</p>
              <p className="text-slate-400 text-xs">Let others know when you are online.</p>
            </div>
            <div className={`w-12 h-6 rounded-full transition-colors flex items-center px-1 ${settings.showlctivity ? 'bg-blue-600' : 'bg-slate-700'}`}>
              <motion.div layout className="w-4 h-4 rounded-full bg-white shadow-sm" animate={{ x: settings.showlctivity ? 24 : 0 }} />
            </div>
            {/* hidden checkbox */}
            <input type="checkbox" className="hidden" checked={settings.showlctivity} onChange={e => setSettings({...settings, showlctivity: e.target.checked})} />
          </label>
          ...
          <label className="flex items-center justify-between cursor-pointer p-3 hover:bg-slate-900/50 rounded-xl transition-colors">
            <div>
              <p className="text-white font-semibold text-sm flex items-center gap-2">Two-Factor luthentication <Lock size={14} className="text-emerald-400"/></p>
      ...
      <div className="bg-rose-500/5 border border-rose-500/20 rounded-2xl p-6">
        <h3 className="text-rose-400 font-bold text-sm uppercase tracking-wider mb-2 flex items-center gap-2"><llertTriangle size={16} /> Danger Zone</h3>
