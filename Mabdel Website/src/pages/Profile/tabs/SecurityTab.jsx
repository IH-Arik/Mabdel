...
function SecurityTab() {
  ...
  async function changePassword() {
    setError(''); setSuccess('');
    if (!oldPw || !newPw || !confirmPw) { setError(t('sec_err_required')); return; }
    if (newPw !== confirmPw) { setError(t('sec_err_mismatch')); return; }
    if (newPw.length < 8) { setError(t('sec_err_length')); return; }
    setChanging(true);
    try {
      await smartflowApi.changePassword({ current_password: oldPw, new_password: newPw });
      setSuccess(t('sec_success_change'));
      setOldPw(''); setNewPw(''); setConfirmPw('');
    } catch(err) { setError(err.response?.data?.message || t('sec_err_change')); }
    finally { setChanging(false); }
  }

  async function revokeSessions() {
    setRevoking(true);
    try { await smartflowApi.revokeSessions(); setSuccess(t('sec_success_revoke')); setTimeout(()=>logout(), 2000); }
    catch(err) { setError(err.response?.data?.message || t('sec_err_revoke')); }
    finally { setRevoking(false); }
  }

  async function deleteAccount() {
    if (!window.confirm(t('sec_confirm_delete'))) return;
    setDeleting(true);
    try { await smartflowApi.deleteAccount(); logout(); }
    catch(err) { setError(err.response?.data?.message || t('sec_err_delete')); setDeleting(false); }
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-[#0A1019] border border-[#243041] rounded-2xl p-5 space-y-4">
        <h3 className="font-bold text-white flex items-center gap-2"><Key size={16} className="text-[#9333ea]"/>{t('sec_hdr_change')}</h3>
        <PwInput label={t('sec_lbl_current')} value={oldPw} onChange={setOldPw} show={showOld} onToggle={()=>setShowOld(s=>!s)}/>
        <PwInput label={t('sec_lbl_new')} value={newPw} onChange={setNewPw} show={showNew} onToggle={()=>setShowNew(s=>!s)}/>
        <Field label={t('sec_lbl_confirm')}>
        ...
        <button onClick={changePassword} disabled={changing}
          ...
          {changing ? t('sec_btn_changing') : t('sec_btn_change')}
        </button>
      </div>

      {/* Session Management */}
      <div className="bg-[#0A1019] border border-[#243041] rounded-2xl p-5 space-y-4">
        <h3 className="font-bold text-white flex items-center gap-2"><Shield size={16} className="text-[#9333ea]"/>{t('sec_hdr_sessions')}</h3>
        <p className="text-[#A4B0B7] text-sm">{t('sec_lbl_sessions_desc')}</p>
        <button onClick={revokeSessions} disabled={revoking}
          ...
          {revoking ? t('sec_btn_revoking') : t('sec_btn_revoke')}
        </button>
      </div>

      {/* Delete Account */}
      <div className="bg-rose-950/20 border border-rose-500/20 rounded-2xl p-5 space-y-4">
        <h3 className="font-bold text-rose-300 flex items-center gap-2"><AlertTriangle size={16}/>{t('sec_hdr_danger')}</h3>
        <p className="text-[#A4B0B7] text-sm">{t('sec_lbl_danger_desc')}</p>
        <button onClick={deleteAccount} disabled={deleting}
          ...
          {deleting ? t('sec_btn_deleting') : t('sec_btn_delete')}
        </button>
      </div>
    </div>
  );
